...
This is the builtin MongoDB id, it's a unique string for each document in the database. It is not a first-class getter on the Model — access it via the generic `get()` method:

```ts
console.log(user.get("_id")); // ObjectId("5f9b1b3c1b9c4e0b4c7b23a1")
```

:::warning Be aware
If you're creating a new user using the new operator, `_id` will be `undefined` until you save the model.
:::
