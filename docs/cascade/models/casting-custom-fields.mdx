...
:::tip Prefer schema for complex transformations
For complex cases—nested objects, conditional logic, cross-field transformations, or embedded model resolution—use the `static schema` approach with `v.embed()` and seal's built-in mutators instead of a raw cast function. See [Validation & Schema](../../validation/schema) for details.
:::
