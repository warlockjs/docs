...
Implement these interface methods:
...
- `get()` - Retrieve value
- `remove()` - Delete key
- `removeNamespace()` - Delete all keys with prefix
- `flush()` - Clear all keys

Use `prepareDataForStorage()` and `parseCachedData()` from BaseCacheDriver for consistent TTL handling.

See [Cache Driver Interface](./cache-driver-interface) for full method signatures and [Base Cache Driver](./base-cache-driver) for available helpers.
