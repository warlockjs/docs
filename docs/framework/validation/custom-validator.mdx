...
Consider a custom validator as the final middleware interceptor before processing the request, it's used to validate the request after the schema validation.

## How it works

After running the validation schema, `Warlock.js` will try to identify the request handler `validate` method, if exists, it will run it, and pass the request and response objects to it.
...
login.validation = {
  validate: async (request: Request, response: Response) => {
    // This runs AFTER schema validation (if any)
