...
A `Warlock.js response` object is attached to every request handler/controller and middleware as well.
...
`Warlock.js response` will parse every data returned in the response, if the value is a plain then it will be sent as-is, if it is an array it will be looped and parsed each value.
...
```ts title="src/app/shared/controllers/sitemap.ts"
import { Request, Response } from "@warlock.js/core";
...
```ts title="src/app/shared/controllers/robots.ts"
import { Request, Response } from "@warlock.js/core";
...
```ts title="src/app/shared/controllers/home.ts"
import { Request, Response } from "@warlock.js/core";
...
For real-time data streaming (e.g., AI responses, live updates):

```ts title="src/app/chat/controllers/stream-response.ts"
import type { RequestHandler } from "@warlock.js/core";

export const streamResponse: RequestHandler = async (request, response) => {
  const stream = response.stream("application/json");

  // Send chunks over time
  stream.send({ message: "Processing...", progress: 0 });

  // Simulate work
  for (let i = 1; i <= 5; i++) {
    await delay(500);
    stream.send({ message: `Step ${i} complete`, progress: i * 20 });
  }

  stream.send({ message: "Done!", progress: 100 });
  stream.end();
};
...
| Method                   | Description            |
| ------------------------ | ---------------------- |
| `stream.send(data)`      | Send a data chunk      |
| `stream.render(element)` | Stream React component |
| `stream.end()`           | Close the stream       |

### Stream Content Types

```ts
// JSON streaming (default)
const stream = response.stream("application/json");
...
Render React components directly in responses:

```ts title="src/app/pages/controllers/home.ts"
import type { RequestHandler } from "@warlock.js/core";
import { HomePage } from "../components/HomePage";

export const home: RequestHandler = async (request, response) => {
  return response.render(<HomePage title="Welcome" user={request.user} />);
};
```

### Streaming React Components

Stream React components for incremental rendering:

```ts title="src/app/reports/controllers/generate-report.ts"
import type { RequestHandler } from "@warlock.js/core";
import { ReportHeader, ReportSection, ReportFooter } from "../components";
...
export const generateReport: RequestHandler = async (request, response) => {
  ...
  stream.render(<ReportHeader title="Sales Report" />);

  // Stream sections as data loads
  const sections = await loadReportSections();
  for (const section of sections) {
    stream.render(<ReportSection data={section} />);
  }

  // Stream footer and end
  stream.render(<ReportFooter />);
  stream.end();
};
```

:::tip
React streaming is ideal for:

- Large reports that render sections progressively
- AI-generated content that streams token by token
