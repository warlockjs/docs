...
Warlock.js embraces the RESTful API design, and provides a set of tools to help you build RESTful APIs.
...
You can use [Generator Z](https://marketplace.visualstudio.com/items?itemName=hassanzohdy.gnz) to generate a Restful controller by right click on the `controllers` directory then `Generate Warlock.js Module` then `Generate Warlock.js Restful Request`.
