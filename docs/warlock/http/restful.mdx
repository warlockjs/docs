...
> If [Cache](/docs/cache/introduction) is enabled then `repository.listCached` method will be used instead of `repository.list`.
...
> If [Cache](/docs/cache/introduction) is enabled then `repository.findCached` method will be used instead of `repository.find`.
