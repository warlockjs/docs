...
But as you can see in the previous example, the post's author has an image, but we didn't define it yet in the user output, most of the time the image is an instance of `Upload` model, which already is built in inside Warlock.js and has as well `UploadOutput` class.
...
Warlock.js has some built-in types that can be used in the output class.
...
When dealing with date objects, Warlock.js will format it in multiple formats, so for each date object is sent to the output, the response shape will be like this:
...
It's not just related to Warlock.js response, you can use it to filter the given data and be sent in a certain format based on the output data.
