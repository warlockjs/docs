...
Make sure that you're already [Configured Cache](/docs/cache/introduction) and it is activated.

:::info Full Cache Documentation
For comprehensive cache API documentation, see [Cache Package Documentation](/docs/cache/introduction). This section covers repository-specific caching features.
:::
...
By default the cache driver of the repository will be the [Cache Manager](/docs/cache/cache-manager), however, you can set a different cache driver for the repository by setting the `cacheDriverName` property to the desired cache:
