...
The output will remain the same, except that [paginationInfo](./../../monpulse/aggregate/fetching#pagination) object will not be returned.
...
Here what we did is we added a custom function callback that receives the passed value which is in our case will be `male` that will be passed as the first argument, and the second argument will be the [Aggregate Query](./../../monpulse/aggregate/model-aggregate) so we can perform a query on it.
...
In some situations, we may need to `perform` a custom query on a particular `list` method, this would not need an additional `filter` to be added to, in this case, we can pass `perform` callback that receives the [Aggregate Query](./../../monpulse/aggregate/model-aggregate) and the options object, this can be passed directly to the `list options`:
