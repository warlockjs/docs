...
After you define [Cache Configurations](./configurations), you can set them using the `setCacheConfigurations` method.
...
:::danger
If the given driver name is not listed in [Cache drivers list](./configurations#drivers), it will throw an error.
:::
...
## More
See [Cache Driver Interface](./cache-driver-interface) for all available methods.
