...
3. [Memory Cache Driver](./memory) — Fast, in-memory cache (resets on restart).
4. [Memory Extended Cache Driver](./memory-extended) — Like memory, but resets TTL on every access (sliding expiration).
5. [LRU Memory Cache Driver](./lru-memory) — In-memory cache with Least Recently Used eviction.
