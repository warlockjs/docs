...
Use `.extend()` to derive a new schema from an existing one — spreading a validator instance doesn't work because Seal validators are class instances and spreading them doesn't merge the internal schema metadata.
...
// ❌ Wrong: spreading a validator instance
// const adminSchema = v.object({
//   ...baseUserSchema,           // This does NOT merge schema fields
//   permissions: v.array(v.string()).required()
// });
```

See [Schema Composition Methods](../object-validator/#schema-composition-methods) for `extend()`, `merge()`, `pick()`, `partial()`, and `without()`.
