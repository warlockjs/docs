...
import { capitalize, rtrim, trim } from "@mongez/reinforcements";
...
export class Postman {
  ...
  public async generate() {
    ...
    for (const route of router.list()) {
      ...
      if (route.middleware) {
        const middlewareThatsHasPostman = route.middleware.filter(
          middleware => (middleware as any).postman !== undefined,
        );

        middlewareThatsHasPostman.forEach(middleware => {
          (middleware as any)?.postman?.onAddingRequest?.({
            ...postmanRequest,
            route,
          });

          if ((middleware as any)?.postman?.onCollectingVariables) {
            PostmanEvents.onCollectingVariables(
              (middleware as any)?.postman?.onCollectingVariables,
            );
          }
        });
      }
    ...
    log.success(
      "postman",
      "generated",
      "Postman collection generated in postman.json",
    );
  }
