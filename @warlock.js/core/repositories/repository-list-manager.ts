...
import type { ModelAggregate, PaginationListing } from "@warlock.js/cascade";
...
> extends BaseRepositoryManager<T, M> {
   ...
   * Get cache driver
   */
  public getCacheDriver(): CacheDriver<any, any> {
   ...
   * Generate cache key
   */
  public cacheKey(key: string | GenericObject, moreOptions?: GenericObject) {
  ...
  public async cacheList({
    ...
    cacheKey?: CachedRepositoryOptions["cacheKey"];
    purge?: boolean;
    expiresAfter?: number;
    ...
    cacheKey = this.cacheKey(cacheKey, options);
