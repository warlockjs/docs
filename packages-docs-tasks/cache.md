# Cache Package — Docs Task

## Your Mission

Fully document the `@warlock.js/cache` package by generating source maps,
analysing the existing docs, and filling every gap. Work autonomously — do not
wait for approval between phases.

> **Note:** Cache has extensive existing docs (`docs/cache/` with 20+ pages).
> Focus on gap analysis and bug-fixing — many pages may need signature fixes.
...
- **Cache source:** `warlock.js/cache/src/` (relative to docs site root)
- **Maps output:** `warlock.js/cache/.maps/src/` (relative to docs site root)
- **Cache docs:** `docs/cache/` (relative to docs site root)
...
1. List all source files under `warlock.js/cache/src/` recursively.
2. List all existing docs pages under `docs/cache/` recursively.
3. Read the source files to understand the package fully — drivers, manager,
   interfaces, utilities, what is exported.
...
the maps against what the docs cover. Write `reports/cache-gap-report.md`:

```markdown
# Cache Documentation Gap Report
...
- New pages match the existing Cache MDX style.
...
```bash
git add docs/cache/ reports/cache-gap-report.md
git commit -m "Cache docs: generate maps, gap report, and complete docs coverage

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"
```

Note: `warlock.js/cache` is a git submodule — if staging `.maps/` fails with
a submodule error, skip it and only commit `docs/cache/` and the report.
