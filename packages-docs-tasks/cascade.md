# Cascade Package — Docs Task

## Your Mission

Fully document the `@warlock.js/cascade` package by generating source maps,
analysing the existing docs, and filling every gap. Work autonomously — do not
wait for approval between phases.

> **Note:** Cascade has extensive existing docs (`docs/cascade/` with many
> subfolders). Focus on gap analysis and bug-fixing rather than writing from scratch.
...
- **Cascade source:** `warlock.js/cascade/src/` (relative to docs site root)
- **Maps output:** `warlock.js/cascade/.maps/src/` (relative to docs site root)
- **Cascade docs:** `docs/cascade/` (relative to docs site root)
...
1. List all source files under `warlock.js/cascade/src/` recursively.
2. List all existing docs pages under `docs/cascade/` recursively.
3. Read the source files to understand the package fully — what it exports,
   how it is structured, what the major features are.
...
the maps against what the docs cover. Write `reports/cascade-gap-report.md`:
...
- New pages match the existing Cascade MDX style.
...
```bash
git add docs/cascade/ reports/cascade-gap-report.md
git commit -m "Cascade docs: generate maps, gap report, and complete docs coverage

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"
```

Note: `warlock.js/cascade` is a git submodule — if staging `.maps/` fails with
a submodule error, skip it and only commit `docs/cascade/` and the report.
