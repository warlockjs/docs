# Logger Package — Docs Task

## Your Mission

Fully document the `@warlock.js/logger` package by generating source maps,
...
- **Logger source:** `warlock.js/logger/src/` (relative to docs site root)
- **Maps output:** `warlock.js/logger/.maps/src/` (relative to docs site root)
- **Logger docs:** `docs/logger/` (relative to docs site root)
...
1. List all source files under `warlock.js/logger/src/` — understand the full
   structure (folders, files, what each likely does from its name).
2. List all existing docs pages under `docs/logger/`.
...
Now read the existing docs pages carefully and compare every exported symbol
from the maps against what the docs cover.

Write the gap report to `reports/logger-gap-report.md`:
...
(wrong method names, invented API, incorrect signatures, missing await, etc.)
...
- New pages match the Herald MDX style.
...
```bash
git add docs/logger/ reports/logger-gap-report.md
git commit -m "Logger docs: maps, gap report, complete coverage

Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"
```

Note: `warlock.js/logger` is a git submodule — if staging `.maps/` fails with
a submodule error, skip it and only commit `docs/logger/` and the report.
